...
public class Constants {
	...
	public static final String PASSWORD_REGULAR_EXPRESSION = "(^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).*$)?(^(?=.*\\d)(?=.*[a-z])(?=.*[@#$%^&+=]).*$)?(^(?=.*\\d)(?=.*[A-Z])(?=.*[@#$%^&+=]).*$)?(^(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).*$)?";
	public static final String ENGLISH_WORDS_REGULAR_EXPRESSION = ".*([a-zA-Z0-9]{3}$)";
